...
void main() {
  group('sign', () {
    test('should produce a signature', () {
      var sig = signature.sign(echash as Uint8List, ecprivkey as Uint8List);
      expect(
          encodeBigInt(sig.r),
          hex.decode(
    ...
    test('should produce a signature for Ropsten testnet', () {
      var sig = signature.sign(echash as Uint8List, ecprivkey as Uint8List, chainId: ropstenChainId);
      expect(
          encodeBigInt(sig.r),
          hex.decode(
              '99e71a99cb2270b8cac5254f9e99b6210c6c10224a1579cf389ef88b20a1abe9'));
  ...
  group('recoverPublicKeyFromSignature', () {
    test('should recover a public key', () {
      ...
      var pubkey = signature.recoverPublicKeyFromSignature(
          signature.ECDSASignature(r, s, v), echash as Uint8List);
      expect(pubkey, signature.privateKeyToPublicKey(ecprivkey as Uint8List));
    });
    ...
    test('should fail on an invalid signature (swapped points)', () {
      ...
      expect(
          () => signature.recoverPublicKeyFromSignature(
              signature.ECDSASignature(s, r, v), echash as Uint8List),
          throwsArgumentError);
    });
  });
  ...
  group('isValidSignature', () {
    ...
    test('should fail when on homestead and s > secp256k1n/2', () {
      ...
      var v = 27;
      expect(
          signature.isValidSignature(r, s, v, homesteadOrLater: true), false);
    });
    test('should not fail when not on homestead but s > secp256k1n/2', () {
      ...
      var v = 27;
      expect(
          signature.isValidSignature(r, s, v, homesteadOrLater: false), true);
    });
    ...
    test('should work otherwise(ropstenChainId=3)', () {
      ...
      var v = 41;
      expect(
          signature.isValidSignature(r, s, v,
              homesteadOrLater: false, chainId: ropstenChainId),
          true);
    });
  });
  ...
  group('message sig', () {
    ...
    test('should return hex strings that the RPC can use', () {
      const v = 27;
      expect(signature.toRpcSig(r, s, v),
